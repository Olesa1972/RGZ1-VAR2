...
foreach ($data as $phone){
	echo '<div class="data">';
	echo '<div class="img">';
	echo	'<img src="'.$phone['mainPhoto']['url'].'" width="280">';	
	echo '</div>';
	echo "<h2><b> Общие характеристики: </b></h1>". "<p>". 'Телефон: '  .htmlspecialchars ($phone['name']). 'id- '. htmlspecialchars ($phone['id']). "<p>".' рейтинг-' .
	htmlspecialchars($phone['rating']). "<p>". 'Описание:' .htmlspecialchars ($phone['description']). "<p>" .' Идентификатор категории: ' .htmlspecialchars( $phone['categoryId']). "<p>". 'цена: '.
	number_format($phone['prices']['amount'],2,',',' '). " ".htmlspecialchars( $phone['prices']['curCode']).
	"<p>". 'Поставщик: '.htmlspecialchars ($phone['vendor']).' код поставщика: '.htmlspecialchars($phone['vendorId'] ).
	"<p>" .'Количество: '.htmlspecialchars( $phone['gradeCount'])."<p> Ссылка на yandex market:" .'<a href='.htmlspecialchars($phone['link'])."'>";
	echo $phone['link'];
	echo "</a>";
	echo '</div>' ;
}
